...
import { ConsentRecord, KenyanLanguage } from '../../types';
import { executeClinicalTriage } from '../../services/channels';
...
export const PatientPortal: React.FC<PatientPortalProps> = ({ currentLanguage, onOpenFHIR, activeTab: activeTabProp }) => {
  const [activeTab, setActiveTab] = useState<'timeline' | 'meds' | 'consent' | 'chat'>('timeline');

  React.useEffect(() => {
    if (activeTabProp) {
      if (activeTabProp === 'medications') setActiveTab('meds');
      else if (activeTabProp === 'consent') setActiveTab('consent');
      else if (activeTabProp === 'messages' || activeTabProp === 'triage') setActiveTab('chat');
  ...
  const [chatMessages, setChatMessages] = useState<Array<{ sender: 'user' | 'ai'; text: string; citation?: string }>>([
    {
      sender: 'ai',
      text: 'Jambo! Mimi ni AfiyaSauti. Ninaweza kukusaidia kuelewa dalili zako kulingana na miongozo ya Wizara ya Afya ya Kenya (MoH). Kumbuka, utambuzi rasmi na maagizo ya dawa yanatolewa na daktari aliyesajiliwa.',
      citation: 'Kenya MoH Clinical Guidelines 2023'
    }
  ]);
  ...
  const [consents, setConsents] = useState<ConsentRecord[]>([
    ...
    {
      consentId: 'CNS-KE-002',
      householdId: 'HH-NRB-7740',
      anonymizedHash: 'dpa2019_hmac_patient_hash_7749',
      consentState: 'ACTIVE',
      purposeCode: 'AMBIENT_MONITORING',
      grantedAt: '2026-09-05T14:30:00Z',
      updatedAt: '2026-09-05T14:30:00Z'
    },
    {
      consentId: 'CNS-KE-003',
      householdId: 'HH-NRB-7740',
      anonymizedHash: 'dpa2019_hmac_patient_hash_7749',
      consentState: 'ACTIVE',
      purposeCode: 'CHP_DISPATCH',
      grantedAt: '2026-09-01T10:00:00Z',
      updatedAt: '2026-09-01T10:00:00Z'
    }
  ]);
    ...
    setConsents((prev) =>
      prev.map((c) =>
        c.consentId === consentId
          ? {
              ...c,
              consentState: c.consentState === 'ACTIVE' ? 'REVOKED' : 'ACTIVE',
    ...
    try {
      ...
      setChatMessages((prev) => [
        ...prev,
        {
          ...
          citation: response.citations[0]?.documentTitle || 'Kenya Ministry of Health Guidelines'
        }
      ]);
    } catch (err) {
      setChatMessages((prev) => [
        ...
        {
          sender: 'ai',
          text: 'Hitilafu ya mtandao. Tafadhali tembelea kituo cha afya kilicho karibu au jaribu tena.'
        }
      ]);
    } finally {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-bold text-white">Patient Health Record (Self-Service)</h2>
                <span className="px-2 py-0.5 rounded text-[11px] bg-emerald-950 text-emerald-300 border border-emerald-800 font-mono">
                  HMAC: 7749...dpa2019
                </span>
              </div>
              <p className="text-xs text-slate-400">
                County: Nairobi (047) | Sub-County: Kibra | Community Unit: Soweto East
              </p>
            </div>
          </div>
